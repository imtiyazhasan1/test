...
//inline policy for enablling ALB
resource "aws_iam_role_policy" "terrafrom_ELB_policy" {
  name = "eks_ELB_policy"
  role = aws_iam_role.eks-worker-node.id
...
//inline policy for EFS
resource "aws_iam_role_policy" "terrafrom_efs_policy" {
  name = "eks_efs_policy"
  role = aws_iam_role.eks-worker-node.id
